...
/// RAG-based First Aid Chatbot screen with session persistence.
...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Future<void> _loadSession(ChatSession session) async {
    setState(() {
      ...
      _messages.clear();
      _messages.add(const _ChatMsg(role: 'assistant', text: 'Loading messages…'));
    });
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _messages.add(_ChatMsg(role: 'assistant', text: result.reply)));
        _scrollToBottom();
      }
    } catch (e) {
      if (mounted) {
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: const LinearGradient(colors: [AppColors.accentGreen, AppColors.accentSecondary]),
              ),
              child: const Icon(Icons.local_hospital_outlined, color: Colors.white, size: 18),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('First Aid Assistant', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
              Text(_sessionId != null ? 'Session active' : 'New session', style: TextStyle(fontSize: 11, color: isDark ? Colors.white38 : Colors.black38)),
            ])),
            IconButton(
        ...
        Container(
          ...
          child: Row(children: [
            ...
            _sending
                ...
                : Container(
                    decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [AppColors.accentPrimary, AppColors.accentSecondary])),
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(
          gradient: isUser ? const LinearGradient(colors: [AppColors.accentPrimary, Color(0xFF5A54E6)]) : null,
          color: isUser ? null : (isDark ? AppColors.darkCard : AppColors.lightCard),
        ...
        child: Text(msg.text, style: TextStyle(color: isUser ? Colors.white : (isDark ? Colors.white70 : Colors.black87), fontSize: 14, height: 1.5)),
      ),
    );
  }
}
...
class _SessionDrawer extends StatefulWidget {
  ...
  const _SessionDrawer({required this.userId, this.currentSessionId, required this.onSelect, required this.onNewChat});
  @override
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const Spacer(),
            TextButton.icon(onPressed: widget.onNewChat, icon: const Icon(Icons.add, size: 18), label: const Text('New chat'), style: TextButton.styleFrom(foregroundColor: AppColors.accentPrimary)),
          ]),
        ),
        Divider(color: isDark ? AppColors.darkBorder : AppColors.lightBorder, height: 1),
        Expanded(
          child: _loading
              ...
              : _sessions.isEmpty
                  ? Center(child: Text('No past sessions', style: TextStyle(color: isDark ? Colors.white38 : Colors.black38)))
                  : ListView.builder(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                      itemCount: _sessions.length,
                      itemBuilder: (_, i) {
                        final s = _sessions[i];
                        final isCurrent = s.sessionId == widget.currentSessionId;
                        return ListTile(
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                          selected: isCurrent,
                          selectedTileColor: AppColors.accentPrimary.withValues(alpha: 0.1),
                          leading: Icon(isCurrent ? Icons.chat : Icons.chat_bubble_outline, size: 20, color: isCurrent ? AppColors.accentPrimary : null),
                          title: Text(s.title ?? 'Untitled', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 14, fontWeight: isCurrent ? FontWeight.w600 : FontWeight.w400)),
                          subtitle: Text(DateFormat('MMM d, h:mm a').format(s.startedAt.toLocal()), style: TextStyle(fontSize: 11, color: isDark ? Colors.white30 : Colors.black26)),

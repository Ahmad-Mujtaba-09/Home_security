# -*- coding: utf-8 -*-
"""FallDetection(2).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1lroN34ZFli6M42_-H_X0u2NzL0QMvab8

HomeSafetyInference.py
======================
Combined inference: TCN fall detection + heuristic + hazard + child safety.

Fixes applied vs previous version (HomeSafetyInference_v1):
  1. TCN ResBlock dropout 0.5→0.2 and head dropout 0.3→0.5 to match training
  2. interpolate_and_smooth rewritten with pure numpy — removes pandas hot-path
      allocation (~40 DataFrame objects/sec at 30fps with 4 tracked persons)
  3. Inactivity timer now persists through LYING (post-fall immobility gap fixed)
  4. PRE_FALL gating no longer bypasses fall_threshold when TCN disagrees
  5. Piped FFmpeg subprocess integrated to replace cv2.VideoWriter for direct compression
  6. kp_data index guard against Ultralytics id/keypoint length mismatch
  7. TCN_WEIGHT + HEURISTIC_WEIGHT validated at init; HEURISTIC_WEIGHT derived
  8. inverted_frames_needed raised from 2 → 5 (~167 ms at 30fps) to reduce
      false positives from bends, kicks, and momentary detections
  9. classify_person_type guards against zero-height bbox triggering infant_thr
 10. gc.collect() removed from frame loop (Python GC is not the bottleneck here)
 11. reset_state() exposed as public method with docstring for live-camera callers
 12. Switched default pose model to YOLOv8s.
 13. Added transparency to display individual TCN and Heuristic probabilities on video and logs.
"""
...
class HomeSafetyInference:
    ...
    def _process_person(
        ...
        if p_bbox is not None:
            ...
            # Fallback: draw label at centroid of valid keypoints so falls
            # are always visible even when the bbox is too small/wide.
            valid_pts = [p for p in kp_pixel if p is not None]
            if valid_pts:
                cx = int(np.mean([p[0] for p in valid_pts]))
                cy = int(np.mean([p[1] for p in valid_pts]))
                cv2.putText(
                    frame,
                    " ".join(parts),
                    (cx, max(0, cy - 10)),

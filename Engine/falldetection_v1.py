...
FEATURE_DIM = 61
KP_CONF = 0.30
TCN_STRIDE = 10  # run TCN every N frames; return cached value between runs
...
def kp_to_bbox(
    kp_list: List, frame_h: int, min_h: float = 0.15, max_ar: float = 1.2
) -> Optional[Tuple]:
...
class HomeSafetyInference:
    ...
    def __init__(
        ...
        self.device_pose_detection = torch.device(
        ...
        logger.info(f"Decimation: {'ON (2x skip)' if frame_decimation else 'OFF'}")
        logger.info(
            f"Motion gate: {'ON (threshold=' + str(motion_threshold) + ')' if motion_gating else 'OFF'}"
        )

        self.pose_model = YOLO(f"yolov8{pose_size}-pose.pt")
        # self._compile_openvino_for_gpu(self.pose_model)
        self.hazard_model = YOLO(hazard_weights)
    ...
    def _init_state_dicts(self) -> None:
        ...
        self.person_type_hist: Dict[
            int, Dict
        ] = {}  # {tid: {"confirmed": str, "streak": int}}
        ...
        self.fps: int = 25  # Updated by run()/run_realtime() from video metadata
        # Temporal hazard buffer: tracks recent detections per hazard class
    ...
    def _compile_openvino_for_gpu(self, yolo_model) -> None:
        """Hacks Ultralytics AutoBackend to bypass PyTorch and force Intel GPU."""
        try:
            # 1. Trigger lazy loading of the backend with a dummy frame
            dummy_frame = np.zeros((320, 320, 3), dtype=np.uint8)
            yolo_model.predict(dummy_frame, verbose=False)

            # 2. Extract the loaded AutoBackend
            if not hasattr(yolo_model, "predictor") or not hasattr(
                yolo_model.predictor, "model"
            ):
                return
            backend = yolo_model.predictor.model

            # 3. Intercept and recompile the OpenVINO core for the Intel GPU
            if hasattr(backend, "core") and hasattr(backend, "model"):
                backend.executable = backend.core.compile_model(
                    backend.model, device_name="GPU"
                )
                logger.info(
                    "Successfully hijacked OpenVINO backend: Bound to Intel Iris Xe GPU."
                )
        except Exception as e:
            logger.warning(f"Could not bind to Intel GPU, falling back to CPU. ({e})")
    ...
    def reset_state(self) -> None:
        ...
        manually when starting a new session, e.g.:
            ...
            for frame in camera_loop():
                annotated, alerts, _ = system.process_frame(frame, frame_idx)
        """
        self._init_state_dicts()
    ...
    def _smooth_type(self, tid: int, raw_type: str, flip_threshold: int = 15) -> str:
        """Sticky lock with hysteresis — O(1) per frame.

        Once confirmed as ADULT, stays ADULT unless `flip_threshold`
        consecutive CHILD classifications override it (and vice versa).
        UNKNOWN is ignored — preserves the current confirmed type.
        """
        state = self.person_type_hist.setdefault(tid, {"confirmed": None, "streak": 0})

        # UNKNOWN = can't see legs → keep whatever we already know
        if raw_type == "UNKNOWN":
            state["streak"] = 0
            return state["confirmed"] or "ADULT"

        # First confident classification for this ID
        if state["confirmed"] is None:
            state["confirmed"] = raw_type
            state["streak"] = 0
            return raw_type

        # Same as confirmed → reset flip streak
        if raw_type == state["confirmed"]:
            state["streak"] = 0
            return state["confirmed"]

        # Different type detected → increment flip streak
        state["streak"] += 1
        if state["streak"] >= flip_threshold:
            # Sustained contradicting signal → flip
            state["confirmed"] = raw_type
            state["streak"] = 0
    ...
    def _temporal_hazard_conf(self, name: str, raw_conf: float) -> float:
        """Boost confidence via temporal consistency (multi-frame evidence).

        Tracks detection history per hazard class over a sliding window.
        If the same hazard is detected across multiple recent frames,
        confidence receives a proportional boost:

            smoothed = raw_conf + MAX_BONUS × (hits / window_size)

        This is a standard temporal evidence aggregation technique.
        """
        WINDOW_SIZE = 5  # Number of recent frames to consider
        MAX_BONUS = 0.30  # Maximum confidence boost at full consistency

        if name not in self._hazard_history:
            self._hazard_history[name] = deque(maxlen=WINDOW_SIZE)

        buf = self._hazard_history[name]
        buf.append(raw_conf)

        # Ratio of frames in the window where this hazard was detected
        consistency = len(buf) / WINDOW_SIZE
        bonus = MAX_BONUS * consistency

        return min(raw_conf + bonus, 0.95)
    ...
    def _detect_and_draw_hazards(self, frame: np.ndarray) -> List[Dict]:
        ...
        if results[0].boxes is not None:
            for box in results[0].boxes:
                ...
                smoothed_conf = self._temporal_hazard_conf(name, conf)
        ...
        # Clear history only after 5 consecutive frames of non-detection
        for prev_name in list(self._hazard_history.keys()):
            if prev_name not in detected_names:
                self._hazard_miss_count[prev_name] = (
                    self._hazard_miss_count.get(prev_name, 0) + 1
                )
                if self._hazard_miss_count[prev_name] >= 5:
                    self._hazard_history[prev_name].clear()
                    self._hazard_miss_count[prev_name] = 0
            else:
                self._hazard_miss_count[prev_name] = 0
    ...
    def _process_person(
        ...
        # Close-to-camera guard: use YOLO detection bbox (full person extent)
        # If bbox height > 60% of frame height, pose is unreliable.
        too_close = False
        if yolo_bbox is not None and fh > 0:
        ...
        # ── Fall alert ────────────────────────────────────────────────────
        if fall_state == "FALL" and person_type != "CHILD" and not too_close:
            alerts.append(
                {
                    "type": "FALL",
                    "pid": tid,
        ...
        # ── Inactivity alarm (video-time-based) ───────────────────────────
        # Timer starts on the first FALL detection and persists through
        # LYING (post-fall immobility). It resets only when the person
        # transitions back to non-floor states (STANDING, WALKING, etc.).
        # Uses VIDEO time (frame_count / fps), not wall-clock time,
        # so it works correctly regardless of processing speed.
        inactivity = False

        if fall_state == "FALL":
            # Confirmed fall — record the frame number if not already running
            if tid not in self.fall_start_time:
                self.fall_start_time[tid] = frame_count
        ...
        if (
            ...
            and (frame_count - self.fall_start_time[tid]) / max(self.fps, 1)
        ...
        # Build detection metadata for Flutter overlay
        det_meta = None
        if p_bbox is not None:
            det_meta = {
                "type": "person",
        ...
        return frame, alerts, det_meta

    def process_frame(
        self, frame: np.ndarray, frame_count: int
    ) -> Tuple[np.ndarray, List[Dict], List[Dict]]:
        """Returns (annotated_frame, alerts, detections_metadata)."""
        fh, fw = frame.shape[:2]
        alerts: List[Dict] = []
        detections: List[Dict] = []  # Lightweight metadata for overlay rendering

        # ── Child module: hazard detection ────────────────────────────────
        hazards = self._detect_and_draw_hazards(frame) if self.child_enabled else []
        for h in hazards:
            x1, y1, x2, y2 = h["bbox"]
            detections.append(
                {
                    ...
                    "label": h["name"],
                    "conf": round(h["conf"], 2),
                    "bbox": [x1 / fw, y1 / fh, x2 / fw, y2 / fh],  # Normalized 0-1
                }
            )

        # ── Elderly module: pose tracking + fall/inactivity ───────────────
        if not self.elderly_enabled:
            self._gc([])
        ...
        results = self.pose_model.track(
            ...
            verbose=False,
            conf=0.25,
            tracker="weights/sticky_tracker.yaml",
            device=self.device_pose_detection,
        )
        active_ids: List[int] = []
        ...
        if (
            ...
            for idx, tid in enumerate(track_ids):
                if idx >= len(kp_data):
                    ...
                    continue
                ybox = results[0].boxes.xyxy[idx].cpu().numpy()
                yolo_bbox = (
                    float(ybox[0]),
                    float(ybox[1]),
                    float(ybox[2]),
                ...
                frame, new_alerts, det_meta = self._process_person(
                    tid, kp_data[idx], frame, fh, fw, frame_count, hazards, yolo_bbox
                )
                alerts.extend(new_alerts)
                if det_meta is not None:
                    detections.append(det_meta)
    ...
    def run(
        ...
        frame_count = 0
        last_annotated = None  # Cache last processed frame for decimation

        # Motion gating: background subtractor
        motion_detector = (
        ...
        while cap.isOpened():
            ...
            # Frame decimation: skip YOLO on odd frames, but keep timeline intact
            if self.frame_decimation and frame_count % 2 != 0:
                # Push None into every tracked person's buffer so
                # interpolate_and_smooth fills the gap and velocities stay correct
                for tid in list(self.kp_buffers.keys()):
                    self.kp_buffers[tid].append(None)
                    # If buffer is full on a skipped frame, still run TCN
                    if (
                        len(self.kp_buffers[tid]) == WINDOW
                        and frame_count % TCN_STRIDE == 0
                    ):
                        self.cached_tcn[tid] = self._get_tcn_prob(tid, frame_count)
                # Reuse last annotated frame so bounding boxes don't disappear
                annotated = last_annotated if last_annotated is not None else frame
                alerts = []
            # Motion gating: skip YOLO if room is still
            elif motion_detector is not None:
                small = cv2.resize(frame, (320, 180))
                mask = motion_detector.apply(small)
                motion_pixels = cv2.countNonZero(mask)
                if motion_pixels <= self.motion_threshold:
                    # Room is still — skip pose/fall but ALWAYS run hazard detection
                    self._detect_and_draw_hazards(frame)
                    annotated = frame
                    alerts = []
                else:
                    annotated, alerts, _ = self.process_frame(frame, frame_count)
            else:
                annotated, alerts, _ = self.process_frame(frame, frame_count)
    ...
    def run_realtime(
        ...
        max_frames: int = 0,
        show_every: int = 60,
        window_name: str = "HomeSafety - Real-Time",
    ) -> List[Dict]:
        """
        Like run(), but displays annotated frames in an OpenCV window
        instead of saving to a file. Press 'q' to quit early.
        """
        tmp_converted = None
        read_path = video_path
        if Path(video_path).suffix.lower() in (".webm", ".avi"):
        ...
        limit = max_frames if max_frames > 0 else total
        logger.info(f"Input  : {video_path}  [{w}x{h} @ {fps}fps  {total} frames]")
        logger.info("Display: Real-time OpenCV window (press 'q' to quit)\n")

        # Create a resizable window and set initial size
        cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
        disp_w = min(w, 1280)
        disp_h = int(disp_w * h / w) if w > 0 else 720
        ...
        logger.info(f"\nDone — {frame_count} frames displayed.")
...
if __name__ == "__main__":
    system = HomeSafetyInference(
        tcn_weights="weights/tcn_fall_best.pt",
        norm_mean="weights/norm_mean.npy",
        norm_std="weights/norm_std.npy",
        hazard_weights="weights/best.pt",
        mode="hybrid",
        fall_threshold=0.65,
    )

    # Real-time display via OpenCV window (press 'q' to quit)
    # system.run_realtime(
    #     video_path="/home/ahmad/My_data/UNI/FYP/New/Engine/testing/test1.mp4",
    # )
    system.run_realtime(
        video_path="/home/ahmad/My_data/UNI/FYP/New/Engine/testing/kitchen.mp4",
    )
